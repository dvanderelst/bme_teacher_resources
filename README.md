...
| Touch and Whiskers | skin receptors, two-point discrimination, whisker sensors on the robot |
...
The **teacher-support bot** is built and deployed: a Streamlit app on Railway, answering from a
Mistral agent whose library is generated from these same chapters, so it cannot drift out of step
with the document a teacher is holding. [BOT-PLAN.md](BOT-PLAN.md) holds the design and the decisions
behind it, and [BUILDING_AND_RELEASING.md](BUILDING_AND_RELEASING.md) explains how to update it.
...
bot/
├── app/              the Streamlit app teachers use
├── lib/              agent, auth, database, moderation, settings
...
├── knowledge/        generated by book/build.sh; gitignored except manifest.json
...
[**BUILDING_AND_RELEASING.md**](BUILDING_AND_RELEASING.md) is the manual for the whole pipeline —
building, updating the bot, and releasing — with a diagram of how the pieces connect and a list of
