...
Connect to the robot using either the Bluetooth dongle or USB cable, as described in [Connecting to the robot](#connecting-to-the-robot), and ensure mBlock is in `Live` mode. If the robot was previously used in Upload mode, reset its firmware first (see [Resetting the firmware](#resetting-the-firmware)).
