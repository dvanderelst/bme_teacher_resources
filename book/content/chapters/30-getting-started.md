...
This chapter assumes we already have mBlock installed or running in our browser. If not, complete [Installing mBlock](#installing-mblock) first, then return here.

## Connection options

To program and control our mBot, connect it to our computer using one of these methods:

1. Using the Bluetooth dongle: A wireless connection with a Makeblock Bluetooth dongle, shown below.
2. Using a USB cable: A wired connection between our computer and the robot, shown below.
...
![Bluetooth dongle with cap off. Symbol is button and indicator](images/getting-started-dongle.jpg)

![USB cable. Square end to robot, flat end to computer](images/getting-started-usb-cable.jpg)

## Pairing the Bluetooth dongle and the robot

> **Note**
>
> With multiple robots in the room, pair each dongle to a specific robot. This ensures the computer using that dongle connects only to its paired robot, allowing us to distinguish identical mBots. Pairing is only needed once per robot.

Follow this sequence precisely: turn all robots off first, then put the dongle into pairing mode, then power on only the robot we are pairing.

1. Switch all robots off, including the one we are pairing. Do this even if only one robot is present.
2. Plug the dongle into a computer.
3. Press the Bluetooth symbol on the top of the dongle. The symbol is both the button and the indicator light. It begins flashing quickly, which means the dongle is in pairing mode and has released any previous pairing. Press it every time we pair, including on a new dongle straight from its packaging: we cannot tell by looking whether a dongle has been paired before, and pressing one that is already in pairing mode does no harm.
4. Switch on the robot we are pairing. Its blue LED starts flashing as well.
...
>
> A slowly flashing dongle looks like a dongle waiting to pair. This is the single most likely reason for a pairing that appears to work but then does not connect. It is why step 3 has us press the button rather than checking the light first.

> **Note**
>
> The button also breaks a pairing: pressing it releases the robot, and that dongle will no longer connect to it until they are paired again. Once a set is paired and labeled, instruct students not to press it. Consider covering it with tape for the rest of the lesson. Remove the tape when we next need to pair that dongle to a different robot.

![Blue LED blinking on robot's Bluetooth module](images/getting-started-draft-1f093a0d.jpg)

![When paired, blue LEDs on dongle and robot stop blinking](images/getting-started-draft-f62edb5b.jpg)

## Checking whether a dongle and a robot are paired

Labels can fall off, and after extended use existing labels may no longer be reliable. We can verify a pair without opening mBlock by observing that paired devices blink when separated from their partner.
...
If separating the pair from either end causes the other to blink, they are correctly paired and we can relabel them confidently. If a light never becomes steady initially, or remains steady after its partner is removed, re-pair them as described above.

## Adding the mBot to mBlock

When writing a program from scratch, we must tell mBlock which robot to target. This is not necessary when starting from one of our programs, which are pre-configured.

A program remembers its device. Add the mBot once, save the program, and it remains available when we reopen the file—on another day or on another computer. Only a brand-new, empty program starts without a device.
...
![Button to add robot to mBlock](images/getting-started-draft-c19638ac.png)
...
When programming our mBot, we can run code in **Live mode** or **Upload mode**.
...
**Live mode:**
The robot remains connected to the computer (via dongle or cable), and mBlock sends commands as the program runs. The robot responds immediately. We can modify and re-test the program without delay. Critically for teaching, we can monitor sensor readings and variable values on screen in real time.
...
![mBlock in browser with mBot in Devices panel](images/getting-started-draft-6bb9e606.png)
...
The instructions below use direct connection as it is simpler and requires no installation—important for locked-down school computers. If the `Devices` panel shows `Switch to direct connection`, click it. If it shows `Switch to mLink`, we are already using direct connection.
...
![mBlock on Linux connected via direct connection in Live mode](images/getting-started-linux-direct.png)
...
The browser then prompts us to select a serial port. Choose the dongle's entry and click `Connect`.

![With direct connection, click Serial, not Bluetooth](images/getting-started-draft-bd535d8d.png)
...
![mBlock installed on Windows with mBot in Devices panel](images/getting-started-draft-45dbe002.png)
...
![Select USB tab, verify port in dropdown, click Connect](images/getting-started-draft-e90f8ba9.png)
...
2. Click `Upload` in the `Mode Switch` row of the `Devices` panel. This is a pair of buttons, `Upload` and `Live`, not a toggle we drag.
...
After uploading a program, the robot cannot be used in Live mode until we reset its firmware, as described in the next section.
...
![Mode Switch buttons and Setting below. No need to change mode before reset](images/getting-started-draft-bf1a09ed.png)

![Select Update Firmware](images/getting-started-draft-8aa4d6fa.png)

![Firmware Version dropdown opened. Online firmware is default for Live mode](images/getting-started-firmware-version.png)

![Leave version as is, click Updates](images/getting-started-draft-05ea3950.png)

## Next step

Once we can connect to the robot, run the example program in [Running your first program](#running-your-first-program). This program verifies the entire setup (software, connection, and robot) in a couple of minutes. It also introduces the pattern used throughout the lessons for opening programs.
