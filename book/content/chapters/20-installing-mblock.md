...
Chrome can communicate directly with the robot via serial port, which our instructions use (see [Direct connection or mLink?](#direct-connection-or-mlink)). On Chrome, we can proceed directly to opening the editor. This is important for locked-down school computers, as the browser route requires no installation at all.
...
Open the block-based editor at [ide.mblock.cc](https://ide.mblock.cc), which we use throughout our lesson plans.
