...
The knowledge set is **not** committed, apart from its `manifest.json`. It regenerates from the
chapters, and it reaches the bot through Mistral rather than through git, so keeping sixteen copies
of the book in the repository bought nothing. The manifest is the exception because the deployed
container reads it to stamp an edition onto every piece of teacher feedback, and a container has no
git history to ask instead.
...
> **One file will be dirty afterwards:** `bot/knowledge/manifest.json`, whose fingerprint changes
> with every build. The generated chapters beside it are ignored, so they no longer show up at all.
...
**3. Commit the regenerated manifest.** Committed *before* tagging, so the tag records which edition
the bot was given. `release.sh` checks that `bot/knowledge/` is the only thing the build touched, and
stops if anything else changed. The generated chapters themselves are not committed: they regenerate
from this tag deterministically, which is what makes leaving them out safe.
...
**A build leaves `manifest.json` dirty.** Its fingerprint changes with every build, so `git status`
shows one modified file afterwards. That is all it should show; the generated chapters are ignored.

This used to be sixteen files, and it was confusing rather than merely untidy: those files embed the
file base *in every image link*, so an ordinary build after a release rewrote every one of them from
`raw/v1.0/…` back to `raw/main/…` and looked like corruption. If you meet that in an old commit,
that is what it was.
...
├── knowledge/            GENERATED by build.sh, and gitignored apart from
│                         manifest.json, which the deployment reads. Do not hand-edit.
