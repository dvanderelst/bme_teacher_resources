...
3. Run the downloaded installer and follow the prompts.
    ...
    Figure: The mBlock installer running. There is nothing to configure; it takes a minute or two. ([image](https://github.com/dvanderelst/bme_teacher_resources/raw/main/book/content/images/installing-mblock-installer.png))
...
4. Near the end, the installer offers to install a **device driver**, which enables computer-to-robot communication. Click `Install`. If you see `Driver Install Failure`, the driver is likely already installed. Click `Uninstall`, then `Install` to retry.
    ...
    Figure: The driver window, which appears separately from the installer. `CH341SER.INF` should appear in the box; click `INSTALL`. Use `UNINSTALL` first if the install fails. ([image](https://github.com/dvanderelst/bme_teacher_resources/raw/main/book/content/images/installing-mblock-driver-setup.png))

    Figure: Confirmation that the driver installed. The message says "drive" rather than "driver" — this is expected. Click `OK`, then close the driver window behind it. ([image](https://github.com/dvanderelst/bme_teacher_resources/raw/main/book/content/images/installing-mblock-driver-installed.png))

5. Click `Finish`.

    Figure: The end of the installation. `Run mBlock` is selected by default, so mBlock opens when you click `Finish`. ([image](https://github.com/dvanderelst/bme_teacher_resources/raw/main/book/content/images/installing-mblock-finish.png))
...
Figure: The mLink installer. Note that the program calls itself mLink2, which is the same as the download page's mLink. ([image](https://github.com/dvanderelst/bme_teacher_resources/raw/main/book/content/images/installing-mblock-mlink-installer.png))

Figure: The end of the mLink installation. As with mBlock, `Run mLink2` is selected by default, so it starts immediately. ([image](https://github.com/dvanderelst/bme_teacher_resources/raw/main/book/content/images/installing-mblock-mlink-finish.png))
...
Figure: The Windows firewall prompt, which appears the first time mLink runs. Select `Private networks`, since mLink and the browser are on the same machine. Click `Allow access`. ([image](https://github.com/dvanderelst/bme_teacher_resources/raw/main/book/content/images/installing-mblock-firewall.png))

Figure: mLink running. This window must remain open; closing it prevents the browser from detecting the robot. It is a launcher, so `Create now` under **mBlock block-based editor** opens the same editor as the next step. ([image](https://github.com/dvanderelst/bme_teacher_resources/raw/main/book/content/images/installing-mblock-mlink-running.png))
