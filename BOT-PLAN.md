...
raises the stakes on the open hardware checks in [TODO.md](TODO.md), and the sonar multiplier was the
sharpest example. It was a sentence in a PDF a reader might skim past; once a bot exists it becomes
the direct answer to "why is my robot hitting the wall?", delivered with confidence and no hedging.
The August 2026 tests settled it — the factor is 1.3, not the 1.25 we had published — which is
exactly the kind of thing that has to be settled *before* the bot is quoting it.

Settle the hardware checks in TODO.md before the bot goes in front of anyone.
...
No chapter said that; chapter 1 stopped at "the only ports with analog inputs" and never
characterised the others. The invention turned out to be true — the board prints the pin numbers
beside each connector, and ports 1 and 2 carry digital pins only — and the chapter now says so. That
it was right is luck, not evidence: it was filling a silence, and the next silence it fills may not
land as well.
